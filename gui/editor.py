...
class CodeEditor(tk.Frame):
    def __init__(self, master, main_app_ref=None):
        ...
        # --- Debug hover tooltip (Watch/Inspect) ---
        self._hover_tooltip = None
        self._hover_after_id = None
        self._last_hover_expr = None
        self.text.bind('<Motion>', self._on_mouse_move_for_debug_tooltip, add='+')
        self.text.bind('<Leave>', self._on_mouse_leave_for_debug_tooltip, add='+')
    ...
    def _ensure_debugger_line_is_current(self):
        """
        Re-applica la riga gialla dopo refresh/syntax-highlight/scroll,
        ma SENZA forzare lo scrolling (altrimenti “blocca” lo scroll dell’editor).
        """
        if self._debugger_active_line is not None:
            # Re-apply: non deve spostare la view
            self.highlight_current_line(
                self._debugger_active_line,
                from_debugger_call=True,
    ...
    def _create_context_menu(self):
        ...
        self.context_menu.add_separator()
        if hasattr(self, "_add_word_to_watch_from_cursor"):
            self.context_menu.add_command(label='Add to Watch', command=self._add_word_to_watch_from_cursor)
    ...
    def _get_word_under_cursor(self):
        """Ritorna l'identificatore Python sotto il cursore (o None)."""
        try:
            idx = self.text.index(tk.INSERT)
            line = self.text.get(f"{idx} linestart", f"{idx} lineend")
            col = int(idx.split(".")[1])

            # trova token python "word"
            ...
            pass
        return None
    def _add_word_to_watch_from_cursor(self):
        """Aggiunge al gruppo Watch la variabile sotto il cursore."""
        try:
            expr = self._get_word_under_cursor()
            if not expr:
                return

            # Main app deve esporre add_watch_expression(expr)
            if self.main_app_ref and hasattr(self.main_app_ref, "add_watch_expression"):
                self.main_app_ref.add_watch_expression(expr)
        except Exception:
            # non deve mai crashare
            return
    # -----------------------------
    # Debug tooltip (hover inspect)
    # -----------------------------
    def _ensure_debug_tooltip(self):
        if getattr(self, "_debug_tooltip", None) is not None:
            return

        self._debug_tooltip = tk.Toplevel(self)
        self._debug_tooltip.withdraw()
        self._debug_tooltip.overrideredirect(True)
        self._debug_tooltip.attributes("-topmost", True)

        self._debug_tooltip_label = tk.Label(
            self._debug_tooltip,
            text="",
            justify="left",
            background="#ffffe0",
        ...
        self._debug_tooltip_label.pack()

        self._debug_tooltip_after = None
        self._last_hover_word = None

    def _hide_debug_tooltip(self):
        try:
            if getattr(self, "_debug_tooltip_after", None):
                self.after_cancel(self._debug_tooltip_after)
        except Exception:
            pass
        self._debug_tooltip_after = None
        self._last_hover_word = None
    ...
    def _on_mouse_leave_for_debug_tooltip(self, event=None):
        self._hide_debug_tooltip()

    def _on_mouse_move_for_debug_tooltip(self, event):
        """
        Mostra tooltip solo quando:
        - il debugger è attivo
        - sei in pausa (paused)
        - il mouse è sopra una parola tipo variabile
        """
        # Tooltip solo in pausa debug
        try:
            if not self.main_app_ref or not getattr(self.main_app_ref, "is_running", False) or not getattr(self.main_app_ref, "paused", False):
                self._hide_debug_tooltip()
                return
        except Exception:
            self._hide_debug_tooltip()
        ...
        self._ensure_debug_tooltip()

        # ricava indice testo sotto mouse
        try:
        ...
        # evita flood se rimani sulla stessa parola
        if word == self._last_hover_word:
            return
        self._last_hover_word = word

        # piccola latenza per non essere “nervoso”
        try:
            if self._debug_tooltip_after:
                self.after_cancel(self._debug_tooltip_after)
        except Exception:
            pass

        def _request_eval():
            # Se main non ha l'API (ancora), mostra solo il nome
            if not hasattr(self.main_app_ref, "evaluate_expression_async"):
                self._show_debug_tooltip_text(event.x_root, event.y_root, f"{word}")
                return

            def _on_result(text):
                # callback può arrivare dopo: ricontrollo pausa
                if not getattr(self.main_app_ref, "paused", False):
                    return
                self._show_debug_tooltip_text(event.x_root, event.y_root, text)

            # chiede al main di valutare `word` nel frame corrente
            try:
                self.main_app_ref.evaluate_expression_async(word, callback=_on_result)
            except Exception:
                self._show_debug_tooltip_text(event.x_root, event.y_root, f"{word}")

        self._debug_tooltip_after = self.after(250, _request_eval)

    def _show_debug_tooltip_text(self, x_root, y_root, text):
        try:
            self._ensure_debug_tooltip()
            self._debug_tooltip_label.config(text=str(text))
            self._debug_tooltip.geometry(f"+{x_root+12}+{y_root+18}")
            self._debug_tooltip.deiconify()
            self._debug_tooltip.lift()
        except Exception:
            pass
    # Alias retro-compatibile per eventuali refusi vecchi
    def _add_word_to_watch_from_curso(self):
        return self._add_word_to_watch_from_cursor()        
    ...
    def toggle_breakpoint(self, line_number):
        """
        Toggle breakpoint e notifica il main anche DURANTE l'esecuzione (runtime).
        Prima era limitato a paused=True, quindi “non prendeva” i breakpoint aggiunti mentre corre.
        """
        ...
        if line_number in self._breakpoints:
            ...
            try:
                if end_idx:
                    self.text.tag_remove('breakpoint_line', start_idx, end_idx)
                else:
                    self.text.tag_remove('breakpoint_line', start_idx)
            except tk.TclError:
                pass
            ...
            try:
                if end_idx:
                    self.text.tag_add('breakpoint_line', start_idx, end_idx)
                else:
                    self.text.tag_add('breakpoint_line', start_idx)
            except tk.TclError:
                pass
            action_taken = "added"
        ...
        print("[editor] notify_breakpoint_change", self.filepath, line_number, action_taken,
            "is_running=", getattr(self.main_app_ref, "is_running", None))

        # Notifica SEMPRE se il debug è in esecuzione (paused o running)
        if self.main_app_ref and hasattr(self.main_app_ref, 'notify_breakpoint_change'):
            try:
                if self.main_app_ref.is_running and self.filepath and action_taken:
    ...
    def highlight_current_line(self, line_number, from_debugger_call=False, scroll_to_line=True):
        """
        Evidenzia la riga corrente (debug line).
        - from_debugger_call=True: setta/aggiorna _debugger_active_line
        - scroll_to_line=False: NON forzare lo scroll (usato nei re-apply post refresh)
        """
        if not self.winfo_exists():
            return

        # When called by the debugger, remember the active line so we can
        # re-apply it after syntax highlighting / scrolling refreshes.
        if from_debugger_call:
            self._debugger_active_line = line_number
        elif self._debugger_active_line is not None:
            # While debugging, don't let "normal" navigation calls accidentally
            # clear the debugger highlight.
            return

        # IMPORTANT: only clear the current highlight if we have a valid line.
        # Otherwise (e.g. go_to_line='?' or None), keep the current debugger line.
        if not isinstance(line_number, int) or line_number <= 0:
            if from_debugger_call:
                try:
        ...
        try:
            ...
            if not scroll_to_line:
                # Re-apply: non cambiare posizione di scroll.
                return

            # Scroll “gentile”: mostra la riga, ma cerca di metterla ~1/3 dall’alto.
            self.text.update_idletasks()
            self.text.see(start_index_str)
            self.text.update_idletasks()

            dline_info = self.text.dlineinfo(start_index_str)
            if not dline_info:
                return

            line_y_in_view = dline_info[1]
            line_height = dline_info[3] if dline_info[3] else self.current_font.metrics('linespace')
            ...
            is_out_bottom = line_y_in_view >= widget_height
            is_out_top = (line_y_in_view + line_height) <= 0
            is_obscured_bottom = (line_y_in_view + line_height > widget_height - margin) and (line_y_in_view < widget_height - margin)
            is_obscured_top = (line_y_in_view < margin) and (line_y_in_view + line_height > margin)

            if is_out_bottom or is_out_top or is_obscured_bottom or is_obscured_top:
                num_lines_in_view_approx = widget_height / line_height
                ...
                if total_lines_in_doc > 0:
                    fraction = (scroll_to_line_num - 1) / float(total_lines_in_doc)
