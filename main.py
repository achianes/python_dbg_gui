...
class MainApplication:
    def __init__(self, root):
        ...
        self._eval_req_id = 0
        self._eval_callbacks = {}   # req_id -> callback
        self.watch_expressions = [] # lista di stringhe
    ...
    def evaluate_expression_async(self, expr, callback=None):
        """
        Valuta `expr` nel frame corrente quando il debugger è in pausa.
        Serve per tooltip (hover) e watch.

        callback: funzione che riceve UNA stringa pronta da mostrare, tipo:
                  "x = 123" oppure "x = <error: ...>"
        """
        expr = (expr or "").strip()

        if not getattr(self, "is_running", False) or not getattr(self, "paused", False):
            if callback:
                callback(f"{expr} = <not paused>")
            return

        # Delegate alla DebuggerApp (GUI-side) che già gestisce eval + callback
        if not getattr(self, "app", None) or not hasattr(self.app, "evaluate_expression_async"):
            if callback:
                callback(f"{expr} = <no debugger>")
            return

        def _cb(expr_in, value_str, success_bool):
            try:
                if callback:
                    if success_bool:
                        callback(f"{expr_in} = {value_str}")
                    else:
                        callback(f"{expr_in} = <error: {value_str}>")
            except Exception:
                pass

        ok = False
        try:
            ok = self.app.evaluate_expression_async(expr, _cb)
        except Exception:
            ok = False

        if not ok and callback:
            callback(f"{expr} = <eval failed>")
    ...
    def _on_eval_expression_result(self, req_id, text):
        """
        Compatibilità: in alcune versioni il backend mandava ('eval_expression_result', req_id, text).
        Se arriva ancora, usiamo i callback vecchi.
        """
        try:
            cb = self._eval_callbacks.pop(req_id, None)
        except Exception:
            cb = None
        if cb:
            try:
    ...
    def add_watch_expression(self, expr):
        """
        Add to Watch: usa la Watch vera del pannello Inspector (self.app.add_watch),
        non una lista legacy che non aggiorna la treeview.
        """
        expr = (expr or "").strip()
        if not expr:
            return

        # Mantieni anche la lista interna se ti serve altrove
        try:
            if expr not in self.watch_expressions:
                self.watch_expressions.append(expr)
        except Exception:
            pass

        if getattr(self, "app", None) and hasattr(self.app, "add_watch"):
            try:
                self.app.add_watch(expr)
            except Exception:
                pass

    def remove_watch_expression(self, expr):
        expr = (expr or "").strip()
        if not expr:
            return
        ...
        # Se hai un metodo di rimozione nel pannello, usalo (se esiste)
        if getattr(self, "app", None) and hasattr(self.app, "remove_watch"):
            try:
                self.app.remove_watch(expr)
            except Exception:
                pass


    def refresh_watch_values(self):
        """
        Rivaluta tutte le watch nel backend (solo se in pausa) e aggiorna il pannello Watch.
        Se la Watch UI gestisce già il refresh internamente, puoi anche non usarlo.
        """
        if not getattr(self, "paused", False):
            return

        # Se la UI Watch ha un refresh, usalo
        if getattr(self, "app", None) and hasattr(self.app, "refresh_watch"):
            try:
        ...
        # Fallback: rivaluta via evaluate_expression_async e prova a propagare se hai hook legacy
        results = {}
        remaining = len(getattr(self, "watch_expressions", []) or [])
        if remaining == 0:
            if hasattr(self, "set_watch_values"):
                try:
                    self.set_watch_values({})
                except Exception:
                    pass
            return
        ...
        def make_cb(expr):
            def _cb(text):
                ...
                if remaining == 0:
                    if hasattr(self, "set_watch_values"):
                        try:
                            self.set_watch_values(results)
                        except Exception:
                            pass
            return _cb

        for expr in list(getattr(self, "watch_expressions", []) or []):
            self.evaluate_expression_async(expr, callback=make_cb(expr))
    ...
    def _do_step(self, step_command_func):
        ...
        # --- FIX UX: Pulisce la riga gialla prima di muovere il debugger ---
        for ed in self.open_tabs.values():
            if ed.winfo_exists():
        ...
        if not self.is_running:
            try:
                ...
                if not cur_tab_id: 
                    messagebox.showwarning("Step", "No file selected.", parent=self.root)
                    return
                editor_widget = self.app.notebook.nametowidget(cur_tab_id)
            except tk.TclError: 
                messagebox.showerror("Step Error", "No editor selected.", parent=self.root)
                return
                
            script_to_run = getattr(editor_widget, 'filepath', None)
            if not script_to_run: 
                messagebox.showwarning("Step", "Selected file has no path. Please save it first.", parent=self.root)
                return
            ...
            if not step_command_func(script_args=script_args_list):
                 self._clear_debug_state()
                 self.status_var.set("Failed to start debugger.")
            self._update_ui_state()
            ...
            if not step_command_func():
                self._set_command_pending(False)
                self.status_var.set("Failed to send step command.")
            self._update_ui_state()
        else: 
            messagebox.showwarning("Step", "Debugger must be paused or not running to step.", parent=self.root)
    ...
    def toggle_run(self):
        ...
        if not self.is_running:
            ...
            # --- FIX: Pulisce la riga gialla all'avvio ---
            for ed in self.open_tabs.values():
                if ed.winfo_exists(): ed.clear_current_line()
            ...
            # --- FIX: Pulisce la riga gialla non appena riprendiamo l'esecuzione ---
            for ed in self.open_tabs.values():
                if ed.winfo_exists(): ed.clear_current_line()
    ...
    def on_breakpoint_hit(self, filepath, line_number):
        ...
        if editor:
            try:
                 self.app.notebook.select(editor); editor.highlight_current_line(line_number, from_debugger_call=True); editor.set_read_only(True)
    ...
    def notify_breakpoint_change(self, filepath: str, line_number: int, action: str):
            """
            Chiamato dal CodeEditor quando toggli un breakpoint.
            Deve inoltrare il comando al DebuggerApp *sul canale comandi* (dbg_conn).
            """
            try:
                if not filepath or not isinstance(line_number, int):
                    return

                if not getattr(self, "is_running", False):
                    return

                # CORREZIONE: usiamo "app" invece di "debugger_app"
                dbg = getattr(self, "app", None)
                if not dbg:
                    return

                if action == "added":
                    dbg.add_breakpoint_runtime(filepath, line_number)
                elif action == "removed":
                    dbg.remove_breakpoint_runtime(filepath, line_number)
